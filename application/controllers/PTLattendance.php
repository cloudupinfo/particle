...
class PTLattendance extends CI_Controller {
    ...
    date_default_timezone_set($Web_settings[0]['timezone']);
        ...
        if($check_exist > 0 ){
             $this->session->set_flashdata('error_message',  display('already_exist'));
              redirect("PTLattendance/single_checkin");
        }
        ...
        redirect("PTLattendance/add_attendance");
        }
         redirect("PTLattendance/add_attendance");
...
$update = $this->db->where('att_id',$this->input->post('att_id',true))
        ...
        $config["base_url"] = base_url('PTLattendance/manage_attendance/');
   ...
   $stay =  $interval->format('%H:%I');
           ...
           redirect("PTLattendance/manage_attendance");
  ...
  function attendance_bulkupload()
    {
        ...
        redirect(base_url('PTLattendance/manage_attendance'));
         }else{
        $this->session->set_userdata(array('error_message'=>'Please Import Only Csv File'));
         redirect(base_url('PTLattendance/add_attendance'));
    }
