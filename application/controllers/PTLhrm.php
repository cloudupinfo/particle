...
class PTLhrm extends CI_Controller {
  ...
  redirect("PTLhrm/manage_designation");
        }
         redirect("PTLhrm/add_designation");
    }
        ...
        if ($this->form_validation->run()) {
         ...
         if ($_FILES['image']['name']) {
            ...
            if (!$this->upload->do_upload('image')) {
                ...
                redirect(base_url('PTLhrm/add_employee'));
            } else {
         ...
         $postData = [
             ...
             if ($this->Hrm_model->create_employee($postData)) { 
                $this->session->set_flashdata('message', display('save_successfully'));
                 redirect("PTLhrm/manage_employee");
            } else {
                $this->session->set_flashdata('error_message',  display('please_try_again'));
                 redirect("PTLhrm/manage_employee");
            }
