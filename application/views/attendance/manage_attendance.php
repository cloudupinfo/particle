  ...
  <table width="100%" class="datatable table table-striped table-bordered table-hover example">
    ...
    <tbody>
      ...
      <?php if($this->permission1->method('manage_attendance','update')->access()){ ?>
                    <a href="<?php echo base_url("PTLattendance/edit_atn_form/".$row['att_id']) ?>" class="btn btn-xs btn-success"><i class="fa fa-edit"></i></a> 
                <?php }?>
    <?php if($this->permission1->method('manage_attendance','delete')->access()){ ?>            
                    <a href="<?php echo base_url("PTLattendance/delete_attendance/".$row['att_id']) ?>" class="btn btn-xs btn-danger" onclick="return confirm('<?php echo display('are_you_sure') ?>') "><i class="fa fa-times" aria-hidden="true"></i></a> 
                      <?php }?>
